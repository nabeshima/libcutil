/**
 * $Id: CHOL.ipp 3 2013-05-20 13:07:23Z cota@upard.org $
...
inline CHOL<DIM, DOUBLE>::CHOL(const CMatrix<DIM, DIM, DOUBLE> &mat) {
  char uplo = 'L';
  int n = DIM;
...
template <int DIM, typename DOUBLE>
inline const CMatrix<DIM, DIM, DOUBLE> &CHOL<DIM, DOUBLE>::L() const {
  return mL;
}

template <int DIM, typename DOUBLE>
inline CHOLm<DIM, DOUBLE>::CHOLm(const CMatrix<DIM, DIM, DOUBLE> &mat)
...
template <int DIM, typename DOUBLE>
inline const CMatrix<DIM, DIM, DOUBLE> &CHOLm<DIM, DOUBLE>::D() const {
  return mD;
}
}
