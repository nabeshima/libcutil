/**
 * $Id: CMatrix.h 3 2013-05-20 13:07:23Z cota@upard.org $
...
class CMatrix {
 ...
 public:
  CMatrix();
  CMatrix(DOUBLE val, bool diag_flag = false);  // NOLINT
  explicit CMatrix(const DOUBLE *vals, bool diag_flag = false);
  ...
  CMatrix<ROWS, COLS, DOUBLE> &insert(const CMatrix<RW, CL, DOUBLE> &mat, int r,
                                      int c);
  ...
  CMatrix<ROWS, COLS, DOUBLE> &operator=(
...
CMatrix<ROWS, COLS, DOUBLE> operator*(const CMatrix<ROWS, MED, DOUBLE> &a,
                                      const CMatrix<MED, COLS, DOUBLE> &b);
...
template <int ROWS, int COLS, typename DOUBLE>
CMatrix<ROWS, COLS, DOUBLE> operator*(DOUBLE b,
...
class CRowVector : public CMatrix<1, COLS, DOUBLE> {
 public:
  CRowVector();
  explicit CRowVector(DOUBLE val);
  explicit CRowVector(const DOUBLE *vals);
  CRowVector(const CMatrix<1, COLS, DOUBLE> &mat);  // NOLINT
};
...
class CColumnVector : public CMatrix<ROWS, 1, DOUBLE> {
 public:
  CColumnVector();
  explicit CColumnVector(DOUBLE val);
  explicit CColumnVector(const DOUBLE *vals);
  CColumnVector(const CMatrix<ROWS, 1, DOUBLE> &mat);  // NOLINT
};
...
class IntVector3 : public CColumnVector<3, int> {
 public:
  ...
  explicit IntVector3(const int *vals);
  IntVector3(const CMatrix<3, 1, int> &vec);  // NOLINT
  IntVector3(const CMatrix<1, 3, int> &vec);  // NOLINT
  explicit IntVector3(int r0, int r1, int r2);
};
}  // namespace cotave
