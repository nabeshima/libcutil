...
inline
DFBFont::DFBFont( const char *font_filename, int font_height ) {
...
inline
bool DFBFont::open( const char *font_filename, int font_height ) { 
  if ( isOpen() ) {
    return false;
  }
...
inline
int DFBFont::getWidth( const char *str ) {
  if ( !isOpen() ) {
    return 0;
  }
  int strwidth;
  font->GetStringWidth( font, 
			str, -1,
			&strwidth );
  return strwidth;
}
