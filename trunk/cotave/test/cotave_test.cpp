...
class ERROR {
public:
  double operator()( const SfM &sfm, const pair< ColumnVector3, ColumnVector3 > &data ) const {
    return ( data.second - ( sfm.scale() * sfm.rotation() * data.first + sfm.translation() ) ).absolute2();
  }
};
...
int main() {
  ...
  cout << RotationMatrix( pos.normalize(), 1.5 ).quaternion().transpose() << endl << endl;
  
  cout << ( RotationMatrix( pos.normalize(), 1.5 ) * pos ).transpose() << endl;
  cout << ( RotationMatrix( pos.normalize(), 1.5 ).rotate( pos ) ).transpose() << endl;
  ...
  mat = Matrix33( 324, 1, 3,
                  1, 34, 5,
                  3, 5, 51 );
  
  cout << mat.cholesky_decomposition( true ) << endl << endl;
  cout << mat.cholesky_decomposition() << endl << endl;
  cout << mat.cholesky_decomposition() * mat.cholesky_decomposition( true ) << endl << endl;
  cout << mat.cholesky_decomposition( true ) * mat.cholesky_decomposition() << endl << endl;
  ...
  cout << "U * S * Vt" << endl << svd.U() * svd.S() * svd.V().transpose() << endl << endl;
  ...
  cout << "mat123 * pinv( mat123 )" << endl << mat123 * mat123.pseudo_inverse() << endl << endl;

  cout << "pinv( mat123 ) * mat123" << endl << mat123.pseudo_inverse() * mat123 << endl << endl;
  ...
  cout << "inv( mat123 ) * mat123" << endl << mat123.inverse() * mat123 << endl << endl;

  cout << "sinv( mat123 ) * mat123" << endl << mat123.sr_inverse( 0.02 ) * mat123 << endl << endl;
...

  {
    ...
    cout << "q1.leftProduct() * q2 " << endl << q1.leftProduct() * q2 << endl << endl;
    cout << "q2.rightProduct() * q1 " << endl << q2.rightProduct() * q1 << endl << endl;
    cout << "q2.leftProduct() * q1 " << endl << q2.leftProduct() * q1 << endl << endl;
    cout << "q1.rightProduct() * q2 " << endl << q1.rightProduct() * q2 << endl << endl;
...

  {
    ...
    cout << " CHOL L*L'" << endl << chol.L() * chol.L().transpose() << endl << endl;

    CHOLm< 3 > cholm( R );
    cout << " CHOLm L" << endl << cholm.L() << endl << endl;
    cout << " CHOLm D'" << endl << cholm.D() << endl << endl;
    cout << " CHOLm L*D*L'" << endl << cholm.L() * cholm.D() * cholm.L().transpose() << endl << endl;
  }

  {
    ...
    CMatrix< 2, 2 > P0;
    P0( 0, 0 ) = 0.1; P0( 0, 1 ) = -0.001;
    P0( 1, 0 ) = -0.001; P0( 1, 1 ) = 0.1;
    ...
    while ( time <= 5.0 ) {
      ...
      cout << time << ";\t " << x.transpose() << ";\t " << ukf.x().transpose() << endl;
...

  {
    ...
    SfM sfm;
    sfm.accumulate( ColumnVector3( 1, 1, 0.2 ), scale * rot * ColumnVector3( 1, 1, 0.2 ) + trans );
    sfm.accumulate( ColumnVector3( 1, 0, 1 ), scale * rot * ColumnVector3( 1, 0, 1 ) + trans );
    sfm.accumulate( ColumnVector3( 1, -1, 1 ), scale * rot * ColumnVector3( 1, -1, 1 ) + trans );
    sfm.accumulate( ColumnVector3( 0, 1, 1 ), scale * rot * ColumnVector3( 0, 1, 1 ) + trans );
    sfm.accumulate( ColumnVector3( 0, -1, 1 ), scale * rot * ColumnVector3( 0, -1, 1 ) + trans );
    sfm.accumulate( ColumnVector3( -1, 1, 1 ), scale * rot * ColumnVector3( -1, 1, 1 ) + trans );
    sfm.accumulate( ColumnVector3( -1, 1, 0 ), scale * rot * ColumnVector3( -1, 1, 0 ) + trans );
    sfm.calculate();
...

  {
    ...
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( 1, 1, 0.2 ), scale * rot * ColumnVector3( 1, 1, 0.2 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( 1, 0, 0 ), scale * rot * ColumnVector3( 1, 0, 0 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( 11, 8, 0 ), scale * rot * ColumnVector3( 11, 8, 0 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( 1, 8, 0.9 ), scale * rot * ColumnVector3( 1, 8, 0.9 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( 0.2, -9, 0.9 ), scale * rot * ColumnVector3( 0.2, -9, 0.9 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( 0.1, -9, 0.91 ), scale * rot * ColumnVector3( 0.1, -9, 0.91 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( 0.2, -9, 0.91 ), scale * rot * ColumnVector3( 0.2, -9, 0.91 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( 102, 9, 1 ), scale * rot * ColumnVector3( 102, 9, 1 ) + trans ) );

    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( -102, 9, 1 ), scale * rot * ColumnVector3( -102, 9, 100 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( -12, 90, 1 ), scale * rot * ColumnVector3( -1, 90, 0 ) + trans ) );
    data.push_back( pair< ColumnVector3, ColumnVector3 >( ColumnVector3( -9, 9, 1 ), scale * rot * ColumnVector3( 1, 9, 100 ) + trans ) );
