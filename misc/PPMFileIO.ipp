/**
 * $Id: PPMFileIO.ipp 3 2013-05-20 13:07:23Z cota@upard.org $
...
inline bool PPMFileIO::write(const std::string &filename,
...
inline bool PPMFileIO::readHeader(const std::string &filename, int *width,
                                  int *height) {
  // open file
  std::ifstream fin;
  ...
  if (!fin || p6 != "P6" || w <= 0 || h <= 0 || max_val != 255) {
    return false;
  }
...
inline bool PPMFileIO::read(const std::string &filename, void *image_data,
