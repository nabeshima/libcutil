/**
 * $Id: STLFileIO.ipp 3 2013-05-20 13:07:23Z cota@upard.org $
...
bool STLFileIO::read(const std::string &filename,
                     ...
                     std::vector<cotave::IntVector3> *indices, bool inversion) {
  vertices->clear();
  indices->clear();
  ...
  if (::strcmp(info, "solid") == 0) {
    ...
    while (inf) {
      inf >> facet >> normal >> n >> outer >> loop >> vertex >> a >> vertex >>
          b >> vertex >> c >> endloop >> endfacet;

      if (facet == "facet" && normal == "normal" && outer == "outer" &&
    ...
    while (inf) {
      ...
      cotave::ColumnVector3 n(data[0], data[1], data[2]),
          a(data[3], data[4], data[5]), b(data[6], data[7], data[8]),
