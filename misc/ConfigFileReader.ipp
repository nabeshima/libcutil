/**
 * $Id: ConfigFileReader.ipp 3 2013-05-20 13:07:23Z cota@upard.org $
...
inline ConfigFileReader::ConfigFileReader() : eof_flag(true) {}

inline ConfigFileReader::ConfigFileReader(const std::string& filename) {
  open(filename);
}

inline ConfigFileReader& ConfigFileReader::operator++() {
  section = "";
  config.clear();
  ...
  while (!fstack.empty()) {
    ...
    while (true) {
      ...
      {
        std::size_t fc = line.find_first_not_of(" \t"),
                    bc = line.find_last_not_of(" \t");

        if (fc != std::string::npos) {
          if (line[fc] == '[' &&
              line[bc] == ']') {  // Sectionフィールドを判定, 取得.
            if (!in_section) {
            ...
            // 親ファイルの位置を保存し,ファイルスタックに積む.
            if (bc - fc > 0) {
              std::string fname = line.substr(fc, bc - fc + 1);

              // 循環参照の禁止
              std::deque<std::pair<std::string, std::ifstream::pos_type> >::
                  const_iterator it = fstack.begin(),
                                 endIt = fstack.end();
              while (it != endIt) {
                if (it->first == fname) {
                  break;
              ...
              if (it == endIt) {  // 現在スタック内にファイルが無いなら良い.
                ...
                fstack.push_back(
                    std::pair<std::string, std::ifstream::pos_type>(fname, 0));
                break;  // 子ファイルを読むループへ
              }
            }
      ...
      while (!tok.isEmpty()) {
        ...
        std::size_t k_fc = buf.find_first_not_of(" \t"),
        ...
        if (ec < buf.size() - 1) {
          ...
          if (v_fc != std::string::npos) {
            std::size_t v_bc = buf.find_last_not_of(" \t\""),
                        v_len = v_bc - v_fc + 1;
...
inline std::map<std::string, std::string>& ConfigFileReader::operator*() {
  return config;
}
...
inline void ConfigFileReader::open(const std::string& filename) {
  fstack.push_back(
      std::pair<std::string, std::ifstream::pos_type>(filename, 0));
  eof_flag = false;
  section = "";
  config.clear();
  ++(*this);
