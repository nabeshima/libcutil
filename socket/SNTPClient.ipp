/**
 * $Id: SNTPClient.ipp 3 2013-05-20 13:07:23Z cota@upard.org $
...
inline timeval SNTPClient::getClock(const Address &ntpserver, int iteration,
  ...
  for (int i = 0; i < iteration; ++i) {
    if (!sock.isSendable(timeout_usec) || !sock.sendAll(com, sizeof(com)) ||
        ::gettimeofday(&sent_tv, NULL) != 0 ||
        !sock.isReceivable(timeout_usec) || !sock.recvAll(data, sizeof(data)) ||
    ...
    double Td = (unsigned int)recv_tv.tv_sec + (double)recv_tv.tv_usec * 1e-6,
           Tc = (ntohl(data[10]) - TIME1970) +
                ((double)ntohl(data[11]) / 4294967296.0),
           Tb = (ntohl(data[8]) - TIME1970) +
                ((double)ntohl(data[9]) / 4294967296.0),
...
inline bool SNTPClient::setClock(const Address &ntpserver, int iteration,
                                 uint64_t timeout_usec) {
