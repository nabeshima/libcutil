/**
 * $Id: IOStream.ipp 3 2013-05-20 13:07:23Z cota@upard.org $
...
template <class T>
inline StreamBuf<T>::StreamBuf(T *t, std::size_t buf_size)
    : t(t), buf_size(buf_size) {
  buffer = new char[buf_size];
}

template <class T>
inline StreamBuf<T>::~StreamBuf() {
  delete[] buffer;
}

template <class T>
inline std::streamsize StreamBuf<T>::xsputn(const char_type *s,
                                            std::streamsize n) {
  if (!t->isOpen()) {
    return 0;
  }
...
template <class T>
inline std::streamsize StreamBuf<T>::xsgetn(char_type *s, std::streamsize n) {
  if (!t->isOpen()) {
    return 0;
  }
...
template <class T>
inline std::streambuf::int_type StreamBuf<T>::overflow(
...
template <class T>
inline std::streambuf::int_type StreamBuf<T>::underflow() {
  if (!t->isOpen()) {
    return std::streambuf::traits_type::eof();
  }
