...
class DFBSurface {
 ...
 public:
  ...
  //   instead of the baseline
  //   DSTF_OUTLINE               0x00000010              enables outline
  //   rendering if loaded font supports it
  ...
  void drawString(const char *str, const DFBFont &font, const DFBColor &color,
                  int x, int y, DFBSurfaceTextFlags flag = DSTF_LEFT);

  void drawLine(const DFBColor &color, int x1, int y1, int x2, int y2);

  void drawRectangle(const DFBColor &color, int x, int y, int width, int height,
                     bool fill_flag = false);

  void drawTriangle(const DFBColor &color, int x1, int y1, int x2, int y2,
                    int x3, int y3, bool fill_flag = false);
};
}  // namespace cutil
